...
global $user, $conn, $dbtables, $logged_in, $globals, $l, $error, $AEF_SESS, $theme;	
	...
	if($globals['login_failed']){
		...
		if($AEF_SESS['login_attempt'] >= $globals['login_failed']){
			...
			reporterror($l['exceeded_attempts_title'], 
						lang_vars($l['exceeded_attempts'], array($AEF_SESS['login_attempt'])));
...
global $user, $conn, $dbtables, $logged_in, $globals, $l, $AEF_SESS, $theme, $error, $tree;
	...
	if(isset($_POST['fpass'])){
		...
		//Send a new password via the email
			...
			$mail[0]['message'] = lang_vars($l['reset_password_mail'], 
											array($username, $resetcode));
...
global $user, $conn, $dbtables, $logged_in, $globals, $l, $AEF_SESS, $theme, $error, $tree;
	...
	if(isset($_POST['fuser'])){
		...
		$mail[0]['message'] = lang_vars($l['reset_username_mail'], 
											array($row['username']));
			...
			//Give a message
			reportmessage($l['fuser_mailsent_title'], $l['fuser_mailsent_title'], '', 
									lang_vars($l['fuser_mailsent'], array($email)));
