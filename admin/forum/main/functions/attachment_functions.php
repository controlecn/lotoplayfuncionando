...
global $user, $conn, $dbtables, $logged_in, $globals, $l, $AEF_SESS, $theme;
	...
	if((mysql_num_rows($qresult) > 0)){
		...
		reporterror( $l['load_mime_error_title'], 
									lang_vars($l['load_mime_error'], array($tid)) );
	...
	foreach($_FILES['attachments']['name'] as $k => $v){
		...
		if(empty($is_uploaded)){
			...
			reporterror($l['upload_error_title'], 
									lang_vars($l['upload_error'], array($name[$k], $tid)));
		...
		if(empty($search)){
			...
			reporterror($l['file_not_allowed_title'], 
									lang_vars($l['file_not_allowed'], array($name[$k], $tid)));
		...
		if($size[$k] > $globals['maxattachsize']){
			...
			reporterror($l['attachment_too_big_title'], 
									lang_vars($l['attachment_too_big'], array($name[$k], $tid)));
		...
		if($totalsize > $globals['maxattachsizepost']){
			...
			reporterror( $l['attachments_size_big_title'], 
									lang_vars($l['attachments_size_big'], array($tid)) );
	...
	if(mysql_affected_rows($conn) < 1){
		...
		reporterror( $l['update_post_error_title'], 
									lang_vars($l['update_post_error'], array($tid)) );
	...
	if(mysql_affected_rows($conn) < 1){
		...
		reporterror( $l['update_topic_error_title'], 
									lang_vars($l['update_topic_error'], array($tid)) );
	...
	if(!empty($upload_error)){
		...
		reporterror( $l['attach_error_title'], lang_vars($l['attach_error'], 
													array(implode(', ', $upload_error), $tid)) );
...
global $user, $conn, $dbtables, $logged_in, $globals, $l, $AEF_SESS, $theme;
	...
	if(!empty($delete_error)){
		...
		reporterror( $l['dettach_error_title'], lang_vars($l['dettach_error'], 
													array(implode(', ', $delete_error), $tid)) );
