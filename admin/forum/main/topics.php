...
global $categories, $forums, $active, $activebots, $anonymous, $guests, $its_in_boards, $board, $topics, $sortby, $sortbylink, $order, $page, $tree;
	...
	$qresult = makequery("SELECT MAX( p.pid ) AS maxpid 
				...
				WHERE t.t_bid = '$fid'
				GROUP BY p.post_tid
				ORDER BY t.t_sticky DESC, ".($sortby=='p.pid'?'maxpid':$sortby)." ".$order."
	...
	//Lets bring out the topics
		...
		$qresult = makequery("SELECT DISTINCT t.*,p.*, ts.username AS starter, u.username, 
				...
				WHERE t.t_bid = '$fid' AND p.pid IN (".$ins.")
