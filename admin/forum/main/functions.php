...
function xhtmlHighlightString($str, $return=false){
	...
	if(!(strpos(substr($str, 0, 3), '<?') === false)) $open = true;
	
	if(!(strpos(substr($str, -3), '?>') === false)) $close = true;
	...
	if(empty($open)) $ret = substr_replace($ret, '', strpos($ret,'&lt;?php'), 8);
		
	//Remove the CLOSE PHP TAG
	if(empty($close)) $ret = substr_replace($ret, '', (strrpos($ret,'?&gt;')), 5);
...
global $globals, $redirect;
	...
	if(!preg_match("/".preg_quote(unhtmlentities(strrev(';tg&a/;tl&.cnI nortcelE;tg&;touq&moc.nortcelena.www//:ptth;touq&=ferh a;tl& 8002-7002 ;ypoc;pma& ;tg&a/;tl&8.0.1 FEA yB derewoP;tg&;touq&moc.nortcelena.www//:ptth;touq&=ferh a;tl&')), "/")."/", $buffer)){
...
function copyright(){
	...
	return '<a href="http://www.anelectron.com">Powered By AEF 1.0.8</a> &copy; 2007-2008 <a href="http://www.anelectron.com">Electron Inc.</a>';
...
global $globals;
	...
	if(substr_count($_SERVER['REQUEST_URI'], '/index.php?') > 0) return false;
	...
	if(preg_match('/\-t(\d{1,20})(\-pg\d{1,20})?\.html$/is', $_SERVER['REQUEST_URI'], $matches)){
		...
		if(!empty($matches[2])) $_GET['tpg'] = str_replace('-pg', '', $matches[2]);
	...
	//No its a Forum
		...
		if(!empty($matches[2])) $_GET['bpg'] = str_replace('-pg', '', $matches[2]);
...
global $globals;
	...
	if(empty($globals['title_in_link'])) $name = '';
...
global $globals, $l, $user;
	...
	if(empty($globals['bannedip']) || empty($_SERVER['REMOTE_ADDR']) || $user['can_admin']) return false;
	...
	foreach($globals['bannedip'] as $k => $v){
		...
		foreach($banned as $bk => $bv){
			...
			//Make it a range
			if(count($temp) == 1) $temp[1] = $temp[0];
			
			//Compare 
			if($ip[$bk] >= min($temp) && $ip[$bk] <= max($temp)) $matches += 1;
...
global $globals, $lower_upper, $upper_lower;
	...
	if($globals['charset'] != 'UTF-8') return strtolower($string);
	
	if(function_exists('mb_strtolower')) return mb_strtolower($string, $globals['charset']);
...
global $globals, $upper_lower, $lower_upper;
	...
	if($globals['charset'] != 'UTF-8') return strtoupper($string);
	
	if(function_exists('mb_strtoupper')) return mb_strtoupper($string, $globals['charset']);
...
global $globals;
	...
	if($globals['charset'] != 'UTF-8') return ucfirst($string);
