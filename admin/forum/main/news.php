...
global $user, $conn, $dbtables, $logged_in, $globals, $l, $AEF_SESS, $theme;
	...
	if(!$user['can_approve_news']) $where['approved'] = "approved = '1'";
	
	//Show ticked news
	if($globals['tickednews']) $where['news'] = "showinticker = '1'";
	
	//Show todays news as well
	if($globals['todaysnews']) $where['news'] = "(".(empty($where['news']) ? '' : $where['news']." OR ")."DATE_FORMAT(FROM_UNIXTIME(time), '%Y%m%d') = '".datify(time(), false, true, 'Ymd')."')";
