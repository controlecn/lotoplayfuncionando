...
if ($cmd=="get") {
	...
	if ($my_points!=0) {
		...
		if ($id==1) {
			...
			SqlQuery("INSERT INTO transfers (userId,transferDate,value,transferType,information,affliateId,buy) VALUES ('".$user_data["id"]."', now(), '$quanto', '2', '".$lang["transfers_convert_points"]."', '0','1')");

			$credits = $credits + $prize_credits;
...
<div class="my_account_points_left">
		...
		<a href="<?=$links["ACCOUNT_POINTS"]?>&cmd=get&id=1" class="blink"><?=$lang["currency_1"]?><?=credits2money($prize_credits)?><?=$lang["currency_2"]?> <?=$lang["account_points_credits"]?></a><br /><br />
		<a href="<?=$links["ACCOUNT_POINTS"]?>&cmd=get&id=2" class="blink"><?=$lang["currency_1"]?><?=credits2money($prize_bonus)?><?=$lang["currency_2"]?> <?=$lang["account_points_bonus"]?></a> <br />
