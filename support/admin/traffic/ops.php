		  ...
		  <tr>
			<td>
				...
				<tr>
					...
					<th nowrap align="center"><b>Qt. clientes</b></th>
				...
				<?php
					for ( $c = 0; $c < count( $admins ); ++$c )
					{
						...
						$consol_status = "Janela fechada" ;
						$bgcolor_consol = "#FFE8E8" ;
						if ( $admin['signal'] == 9 )
						{
							$consol_status = "Janela Aberta" ;
						...
						$request_string = "<font color=#BCBCBC>[ Conversar ]</font>" ;
						if ( $admin['userID'] == $_SESSION['session_admin'][$sid]['admin_id'] )
							$request_string = "&nbsp;" ;
						else if ( $online_status == "Online" )
							$request_string = "<a href=\"JavaScript:void(0)\" OnClick=\"window.open( '$BASE_URL/request.php?action=op2op&userid=". $_SESSION['session_admin'][$sid]['admin_id']."&op2op=$admin[userID]&l=$l&x=$x&deptid=0&pagex=', 'op2op', 'scrollbars=no,menubar=no,resizable=0,location=no,screenX=50,screenY=100,width=450,height=360' )\">[ Conversar ]</a>" ;
