<?php
	...
	if ( $winapp )
		$close_flag = -1 ;
	$logout_string = "&nbsp; <a href=\"JavaScript:void(0)\" OnClick=\"do_logout($winapp, $close_flag)\"><img src=\"../images/extra/btn_logout.gif\" width=\"41\" height=\"18\" border=\"0\" alt=\"logout\"></a>" ;
...
<tr>
	...
	<td align="center">
		...
		<input type="button" value="Close the window at the other location." OnClick="parent.window.do_kill()" class="button"> or <a href="JavaScript:do_window_close()">Cancel</a>
		<?php endif ; ?>
	</td>
	</form>
</tr>
...
<tr>
	...
	<td align="center">
		<br>
		<b><big>Sending close command to other window ...</big></b>
		<p>
		Re-logging in <input type="text" name="ticker" size=2 maxlength=3 disabled> second(s).  To cancel <a href="JavaScript:history.go(-1)">click here</a>.
	</td>
	</form>
</tr>
<?php else: ?>
...
<tr>
	...
	<tr>
		...
		<th class="th1" align="left">Pergunta</th>
		<th width="5" class="th1">&nbsp;</th>
	...
	<?php
		for ( $c = 0; $c < count( $chat_requests ); ++$c )
		{
			...
			$transfer_string = "&nbsp;" ;
			if ( $request['tflag'] == 1 )
				$transfer_string = "<img src=\"$BASE_URL/images/extra/polled.gif\" width=10 height=10 alt=\"previous operator not available - call auto polled\">" ;
			else if ( $request['tflag'] == 2 )
				$transfer_string = "<img src=\"$BASE_URL/images/extra/busy.gif\" width=10 height=10 alt=\"previous operator 'busy' - call polled\">" ;
			else if ( $request['tflag'] == 3 )
				$transfer_string = "<img src=\"$BASE_URL/images/extra/transfer.gif\" width=10 height=10 alt=\"operator transferred call\">" ;
			...
			<tr>
				...
				<td width=\"5\">&nbsp;</td>
				<td nowrap align=\"center\"><a href=\"JavaScript:void(0)\" OnClick=\"window.open('$BASE_URL/request.php?action=accept&sessionid=$request[sessionID]&requestid=$request[requestID]&sid=$sid&userid=".$_SESSION['session_admin'][$sid]['admin_id']."&l=".$_SESSION['session_admin'][$sid]['asp_login']."&x=".$_SESSION['session_admin'][$sid]['aspID']."', '$request[created]', 'status=no,scrollbars=no,menubar=no,resizable=no,location=no,width=450,height=$chat_win_height,screenX=50,screenY=100')\"><b>ABRIR</b></a><!-- &nbsp; | &nbsp; <a href=\"JavaScript:parent.window.do_reject($request[requestID], $request[sessionID])\">FECHAR</a>--></td>
				<td width=\"5\">&nbsp;</td>
			</tr>
			" ;
