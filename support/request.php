<?php
	...
	if ($_COOKIE["session2"]) {
		die ("<b>Departamento temporiariamente indisponivel. Por favor, tente novamente em alguns minutos.</b>");
	}
	...
	if ( !Util_DIR_CheckDir( ".", $l ) || !isset( $_SESSION['session_chat'] ) )
	{
		...
		{
			HEADER( "location: http://www.reidobingo-net.umbler.net" ) ;
			exit ;
		}
		else
		{
			...
			if ( $sessionid && $requestid )
			{
				...
				if ( !file_exists( "./web/chatsessions/".$_SESSION['session_chat'][$sid]['chatfile_put'] ) )
				{
					...
					if ( ( $total_requests > 0 ) && $deptinfo['show_que'] )
						$status_string .= "<br><br> Nossos consultores estão em atendimento, por favor aguarde. Você é o número <b>$total_requests</b> da fila." ;
...
if ($session_id) {
	...
	$my_result = mysql_query("SELECT userId,userType FROM lotoplay.sessions WHERE sessionId = '$session_id'", $mysql_link);
	$count = mysql_num_rows($my_result);
	...
	if ($count!=0) {
		...
		if ($user_id!=0) {
			...
			if ($user_type==1) {
				// Cliente
				$my_result = mysql_query("SELECT username,email FROM lotoplay.users WHERE id = $user_id", $mysql_link);
				...
				$my_result = mysql_query("SELECT login,email FROM lotoplay.users_affliates WHERE id = $user_id", $mysql_link);
...
if ($logged==0) {
	...
	$lp_name = "Visitante";
	$lp_email = "visitante@lotoplay.com";
...
<!--
	...
	function init()
	{
		...
		if ( !document.createElement && !document.createElementNS ) self.location.href = "browser.php";
		if ( !initxmlhttp() ) self.location.href = "browser.php?xmlhttp=1";
		open_chat() ;
	}
...
<div id="main">
	...
	<div id="inputarea">
		<fieldset>
			...
			<input type="hidden" id="user_name" name="from_screen_name"  value="<?php echo $lp_name ?>" />
			<input type="hidden" id="email" name="email" value="<?php echo $lp_email ?>" />
			...
			<input type="button" id="send" name="send" value="Entrar" onclick="return do_submit();" />
			<?php endif ; ?>
