...
<form name="counter">
Transferência automatica: <big><b><?php echo "$user[name]" ?></b></big> de <?php echo $department['name'] ?> em <input type="text" name="counter_value" value="" class="input" size=2> segundos...
<br><br>
[ <a href="chat_admin_transfer.php?sessionid=<?php echo $sessionid ?>&sid=<?php echo $sid ?>&requestid=<?php echo $requestid ?>&userid=<?php echo $userid ?>&action=transfer_doit&deptid=<?php echo $deptid ?>">Transfer Now</a> ]
&nbsp;
[ <a href="chat_admin_transfer.php?sessionid=<?php echo $sessionid ?>&sid=<?php echo $sid ?>&requestid=<?php echo $requestid ?>&userid=<?php echo $userid ?>&rand=<?php echo $rand ?>">STOP Transfer</a> ]
</form>


<?php elseif ( $action == "transfer_verify" ): ?>
<b><?php echo $user['name'] ?></b> esta <b>OFFLINE</b>.
<br><br>
<a href="chat_admin_transfer.php?sessionid=<?php echo $sessionid ?>&sid=<?php echo $sid ?>&requestid=<?php echo $requestid ?>&userid=<?php echo $userid ?>&rand=<?php echo $rand ?>">Go back</a>


<?php elseif ( $action == "transfer_doit" ): ?>
<big><b>Transferência foi successo!</b></big>
...
<?php
	for ( $c = 0; $c < count( $departments ); ++$c )
	{
		...
		for ( $c2 = 0; $c2 < count( $department_users ); ++$c2 )
		{
			...
			{
				$status = "online" ;
				$activity = "$total_sessions clientes" ;
			}
