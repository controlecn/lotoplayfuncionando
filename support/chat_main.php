...
<!--
	...
	function write_typing( name )
	{
		...
		var id = "typing" ;
		document.getElementById(id).innerHTML = name + " está digitando..." ;
	}
//-->
</script>
...
<div id="main">
	...
	<div id="status">
		...
		<span id="onlinetime" style="visibility:hidden;">Duração: <span id="timer">00:00</span></span>
	</div>
	
	<div id="close"><a href="javascript:closeWindow(1);" title="Close window and end conversation">Fechar</a></div>
	
	<div id="sound_print">
		<a href="javascript:void(0)" OnClick="window.open('admin/view_transcript.php?x=<?php echo $session_chat[$sid]['aspID'] ?>&l=<?php echo $session_chat[$sid]['asp_login'] ?>&chat_session=<?php echo $sessionid ?>&deptid=<?php echo $session_chat[$sid]['deptid'] ?>&sid=<?php echo $sid ?>&requestid=<?php echo $requestid ?>&action=view&theme_admin=<?php echo ( $session_chat[$sid]['isadmin'] ) ? $session_chat[$sid]['theme'] : "" ?>', 'newwin', 'status=no,scrollbars=no,menubar=no,toolbar=no,resizable=yes,location=no,width=450,height=360')" title="Printer friendly version" class="print">Versão para impressão</a>&nbsp;&nbsp;
		<a href="javascript:fontSize();" title="Mude o tamanho do texto" class="font">Tamanho do texto</a>&nbsp;&nbsp;
		<a href="javascript:muteSound();" title="Deshabilitar o som" class="soundOn" id="sound">Som</a>
	</div>
	...
	?>
	<div id="copyright">&copy; LotoPlay.com</div>
	<?php endif ; ?>
...
<div id="operator">
	<div><?
		...
		$lp_sexo = 0;
		$lp_username = substr($session_chat[$sid]['visitor_name'], strpos($session_chat[$sid]['visitor_name'], ">")+1, strlen($session_chat[$sid]['visitor_name']));
	
		$mysql_link = mysql_connect("reidobingo-net.umbler.net", "bingo", "xxx123");
				
		$my_result = mysql_query("SELECT sex FROM lotoplay.users WHERE username = '$lp_username'", $mysql_link);
	...
	?>
		...
		<select name="responses" id="responses">
			...
			if ($lp_sexo==0) {
				...
				if ($lp_sexo==1) {	// Homem
					...
					for ( $c = 0; $c < count( $canneds ); ++$c )
					{
						...
						if (substr_count($canned_name, "SENHORA")==0) {
							print "		<option value=\"$canned_message\">$canned_name</option>\n" ;
						}
					...
					for ( $c = 0; $c < count( $canneds ); ++$c )
					{
						...
						if (substr($canned_name, -6)!="SENHOR") {
							print "		<option value=\"$canned_message\">$canned_name</option>\n" ;
						}
					}
			...
			?>
		</select> <input name="submit_response" type="button" value="OK" class="go" OnClick="do_select( document.chatform.responses )" />&nbsp;&nbsp;
		...
		<select name="replies" id="replies">
			...
			?>
		</select> <input name="submit_replies" type="button" value="OK" class="go" OnClick="do_select( document.chatform.replies )" />
	</div>
	...
	<div id="tabs">
		<ul>
			...
			<li id="creditos"><a href="chat_admin_vinfo.php?sessionid=<?php echo $sessionid ?>&sid=<?php echo $sid ?>&requestid=<?php echo $requestid ?>&action=creditos" onclick="swapTabs('creditos');" target="foperator"><span>Creditos</span></a></li>
			<li id="alerts"><a href="chat_admin_vinfo.php?sessionid=<?php echo $sessionid ?>&sid=<?php echo $sid ?>&requestid=<?php echo $requestid ?>&action=alerts" onclick="swapTabs('alerts');" target="foperator"><span>Tapiação</span></a></li>
			<li id="comments"><a href="chat_admin_vinfo.php?sessionid=<?php echo $sessionid ?>&sid=<?php echo $sid ?>&requestid=<?php echo $requestid ?>&action=comments" onclick="swapTabs('comments');" target="foperator"><span>Commentários</span></a></li>
			<li id="call"><a href="chat_admin_transfer.php?sessionid=<?php echo $sessionid ?>&sid=<?php echo $sid ?>&requestid=<?php echo $requestid ?>&action=transfer" onclick="swapTabs('call');" target="foperator"><span>Transferir</span></a></li>
