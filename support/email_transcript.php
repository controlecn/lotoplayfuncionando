...
<!--
	...
	function checkifloaded()
	{
		...
		if ( loaded == 1 )
			...
			alert( "Erro: Conversa não enviada. Por favor tente de novo." ) ;
	}
	...
	function doit()
	{
		...
		document.form.submitbutton.value = "Um momento.  Enviando ..." ;
...
<div id="main">
	...
	<div id="inputarea">
		<fieldset>
			...
			<p>Se você quer receber a copia dessa conversa para o seu e-mail,
prencha o campo abaixo com seu endereço de email e clique no botão. (Opcional)</p>
			<dl>
				<dt><label for="email">Seu e-mail</label></dt>
			...
			<dl>
				<dt>&nbsp;</dt>
				<dd><br><input type="button" value="Enviar e fechar a janela" onclick="do_submit()" class="button" name="submitbutton"></dd>
			</dl>
			<dl></dl>
			<dl>
				<dt>&nbsp;</dt>
				<dd><br><a href="javascript:void(0)" OnClick="window.open('admin/view_transcript.php?x=<?php echo $session_chat[$sid]['aspID'] ?>&l=<?php echo $session_chat[$sid]['asp_login'] ?>&chat_session=<?php echo $sessionid ?>&deptid=<?php echo $session_chat[$sid]['deptid'] ?>&sid=<?php echo $sid ?>&requestid=<?php echo $requestid ?>&action=view', 'transcriptwin', 'status=no,scrollbars=no,menubar=no,toolbar=no,resizable=yes,location=no,width=450,height=360')" title="Versão para impressão" class="print">Versão para impressão</a></dd>
			</dl>
		</fieldset>
